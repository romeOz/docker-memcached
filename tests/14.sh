...
echo
echo
echo "-- Building Memcahed 1.4 image"
docker build -t memcahed-14 ../1.4/
docker network create mem_net

echo
echo "-- Testing memcached is running"

docker run --name base_1 -d --net mem_net memcahed-14; sleep 10
docker run --name base_2 -d --net mem_net memcahed-14; sleep 10
